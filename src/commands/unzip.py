...
def validate_archive_format(archive_path: str) -> tuple:  # Проверяем формат архива
    ...
    if archive_object.suffix.lower() != '.zip':
        return False, "ERROR: Файл не является ZIP-архивом (должен иметь расширение .zip)"
    return True, archive_path
...
def extract_zip_archive(archive_path: str, catalog_path: str) -> tuple:  # Извлекаем файлы из ZIP архива
    try:
        with zipfile.ZipFile(archive_path, 'r') as zip_file:
            zip_file.extractall(catalog_path)  # Извлекаем все файлы из архива в целевой каталог
        return True, f"Архив распакован: {archive_path} -> {catalog_path}"
    except zipfile.BadZipFile:
        return False, "ERROR: Архив поврежден или не является корректным ZIP-файлом"
    except Exception as e:
